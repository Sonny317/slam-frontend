...
  };
  ...
  const handleGoogleSignup = async () => {
    ...
    // 약관 동의 완료 후 실제 Google OAuth 진행
    await proceedWithGoogleOAuth();
  };

  // ✅ 실제 Google OAuth 진행 함수
  const proceedWithGoogleOAuth = async () => {
    ...
    try {
      ...
      if (response.data && response.data.authUrl) {
        ...
        alert("Google login feature is not ready yet. Please use regular signup.");
      }
    } catch (error) {
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[80vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold">Terms Agreement Required for Google Login</h2>
            <button 
          ...
          <div className="space-y-4 text-sm text-gray-700 mb-6">
            <p className="text-red-600 font-medium">⚠️ You must agree to the following terms before proceeding with Google login:</p>
            ...
            <div className="space-y-3">
              <label className="flex items-center">
                ...
                />
                <span className="ml-2">I agree to the <button type="button" onClick={() => setShowTermsModal(true)} className="font-medium text-blue-600 hover:text-blue-800 underline">Terms of Service</button> (Required)</span>
              </label>
              ...
              <label className="flex items-center">
                <input 
                  ...
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" 
                />
                <span className="ml-2">I agree to the <button type="button" onClick={() => setShowPrivacyModal(true)} className="font-medium text-blue-600 hover:text-blue-800 underline">Privacy Policy</button> (Required)</span>
              </label>
            ...
            <button 
              onClick={() => {
                if (formData.termsOfServiceAgreed && formData.privacyPolicyAgreed && formData.eventPhotoAgreed) {
                  ...
                  // 약관 동의 완료 후 구글 로그인 실행
                  proceedWithGoogleOAuth();
                } else {
...
        </form>
      ...
      {showGoogleTermsModal && <GoogleTermsModal />}
    </div>
  );
}